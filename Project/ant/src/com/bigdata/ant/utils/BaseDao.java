...
public abstract class BaseDao<T, PK extends Serializable> {
	 ...
	 * @Description: TODO(根据hql,按条件投影查询，查询前几个)
	 * @author 成琼
	 * @param:@param hql hql语句
	 * @param:@param params hql语句中占位符对应的参数
	 * @param:@param num 查询数目
	 * @param:@return
	 * @param:@throws Exception (参数)
	 * @return:List<Object[]>( List集合，集合中数据是对象数组)
	 */
	@SuppressWarnings("unchecked")
	public List<Object[]> findByProjection(String hql, Object[] params, int num) throws Exception {
		Query query = this.sessionFactory.getCurrentSession().createQuery(hql);
		if (params != null && params.length > 0) {
			for (int i = 0; i < params.length; i++)
				query.setParameter(i, params[i]);
